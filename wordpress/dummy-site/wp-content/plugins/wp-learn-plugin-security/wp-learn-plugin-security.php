...
function wp_learn_form_shortcode( $atts ) {
	...
	<div id="wp_learn_form" class="<?php echo esc_attr($atts['class']) ?>">
		<form method="post">
			...
			<?php
			wp_nonce_field('wp_learn_form_nonce_action', 'wp_learn_form_nonce_action');
			?>
			<div>
...
function wp_learn_maybe_process_form() {
	...
	if (isset($_POST['wp_learn_form_nonce_action']) === false || wp_verify_nonce($_POST['wp_learn_form_nonce_action'], 'wp_learn_form_nonce_action') === false) {
		return wp_redirect( WPLEARN_ERROR_PAGE_SLUG );
	}
