let shapeValue = 'box'
let coloringValue = 'plainColor'
let offsetValue = 0

const shadersOptions = {
    vertex: `
...
function Render() {
    ...
    const shaderProgram = initShaderProgram(gl, shadersOptions.vertex, shadersOptions.fragment(shadersOptions[shapeValue], shadersOptions[coloringValue]))
...
function AttachToInputElements() {
    const shapeInput = document.getElementById("shape") as HTMLSelectElement | null
    if (shapeInput !== null) {
        shapeValue = shapeInput.value
        shapeInput.addEventListener("change", (e: Event) => {
    ...
    const coloringInput = document.getElementById("coloring") as HTMLSelectElement | null
    if (coloringInput !== null) {
        coloringValue = coloringInput.value
        coloringInput.addEventListener("change", (e: Event) => {
            const target = e.target as HTMLSelectElement
            coloringValue = target.value
            Render()
        })
    }
    const offsetInput = document.getElementById("offset") as HTMLSelectElement | null
    if (offsetInput !== null) {
...
function initShaderProgram(gl: WebGLRenderingContext, vertexShaderSource: string, fragmentShaderSource: string) {
